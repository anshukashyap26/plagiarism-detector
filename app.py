...
# ---- Optional modules (guarded imports) ----
try:
...
st.caption(
    ...
    "with strict evidence (Google CSE), and check simple AI-text heuristics."
)
...
with tabs[0]:
    ...
    if "KMP" in algo or "Rabin" in algo:
        pattern = st.text_input("Pattern to search (for exact-match modes)", "")
...
with tabs[1]:
    ...
    if run_web:
        if scan_text_against_web is None:
            ...
            for m in matches:
                ...
                by   = m.get("verified_by")
                extra = []
                if isinstance(frac, (int, float)):
                    extra.append(f"match:{frac:.2f}")
                if by:
                    extra.append(f"by:{by}")
    ...
    with st.expander("🔧 Debug: Google CSE health check"):
        ...
        api_key_masked = (api_key[:6] + "…") if api_key else None
        st.write({"GOOGLE_API_KEY_present": bool(api_key), "GOOGLE_CX_present": bool(cx_val),
                  "GOOGLE_API_KEY_masked": api_key_masked})
        test_q = st.text_input("Test query", value="internet")

        if st.button("Run sanity search"):
            import requests
            params = {"key": api_key, "cx": cx_val, "q": test_q, "num": 3}
            try:
